  ...
  const executeWithRetry = useCallback(
    async (variables: TVariables, attempt: number = 0): Promise<TData> => {
      try {
        ...
        const result = await Promise.race([mutationFn(variables), timeoutPromise]);
...
export function wrapMutationWithResilience<TData, TVariables>(
  ...
  const resilientMutateAsync = async (variables: TVariables, attempt: number = 0): Promise<TData> => {
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('Request timed out')), timeout);
      });

      const result = await Promise.race([originalMutation.mutateAsync(variables), timeoutPromise]);
