          ...
          {sortedWeeks.map((weekStart) => {
            ...
            return (
              ...
              >
                ...
                >
                  Week: {formatWeekRange(weekStart)} | Gross: {formatCurrency(weekGross)} | Deposits:{' '}
                  {formatCurrency(weekDeposits)}
                </div>
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 font-medium" style={{ color: colors.brown }}>
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  title={
                                    entry.excluded_from_average
                                      ? 'Included in averages (click to exclude)'
                                      : 'Exclude from averages (festival/event day)'
                                  }
                            ...
                            <td
                              ...
                              style={{ color: colors.brown, opacity: entry.excluded_from_average ? 0.5 : 1 }}
                            >
                              {formatCurrency(entry.gross_revenue)}
                            </td>
                            <td className="px-4 py-3 text-right" style={{ color: colors.brown }}>
                              {formatCurrency(entry.cash_sales)}
                            </td>
                            <td className="px-4 py-3 text-right" style={{ color: colors.brown }}>
                              {formatCurrency(entry.tip_pool)}
                            </td>
                            <td className="px-4 py-3 text-right" style={{ color: colors.brown }}>
                              {formatCurrency(entry.actual_deposit)}
                            </td>
                            <td className="px-4 py-3 text-right" style={{ color: colors.brownLight }}>
                              {formatCurrency(entry.calculated_deposit)}
                            </td>
                            <td
                              className="px-4 py-3 text-right font-medium"
                              style={{
                                color:
                                  Math.abs(entryDiff) < 0.01 ? colors.green : entryDiff > 0 ? '#eab308' : '#ef4444',
                              }}
                            >
                              ...
                              <div className="flex items-center justify-center gap-1">
                                ...
                                <Button
                                  ...
                                  title={
                                    entry.excluded_from_average
                                      ? 'Include in averages'
                                      : 'Exclude from averages (outlier day)'
                                  }
                                  data-testid={`button-exclude-action-${entry.id}`}
                                >
                                  <Flag
                                    className="h-4 w-4"
                                    style={{ color: entry.excluded_from_average ? '#f59e0b' : colors.creamDark }}
                                  />
                                </Button>
                                <Button
                                  size="icon"
