...
export function BottomTabBar() {
  const { mobileTabItems, primaryItems, settingsItems, utilityItems } = useNavigation();
  ...
  const needsMoreTab = syntheticMore != null || realTabItems.length > MAX_VISIBLE_TABS;
  ...
  const overflowTabItems = needsMoreTab ? realTabItems.slice(MAX_VISIBLE_TABS - 1) : [];

  const moreSheetItems = primaryItems.filter((item) => !visibleTabs.some((tab) => tab.id === item.id));
  ...
  const isMoreActive = needsMoreTab && overflowTabItems.some((item) => isTabActive(item.href));
        ...
        <div className="flex justify-around items-center">
          ...
          {needsMoreTab && (
            <button
              ...
              style={{
                color: isMoreActive ? 'var(--color-primary)' : 'var(--color-secondary)',
              }}
            >
      ...
      <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
        <SheetContent side="bottom" className="max-h-[70vh] overflow-y-auto">
          ...
          {moreSheetItems.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 px-2">Pages</p>
          ...
          {settingsItems && settingsItems.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 px-2">Settings</p>
              <div className="space-y-1">
          ...
          {utilityItems && utilityItems.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 px-2">More</p>
              <div className="space-y-1">
