...
import { Plus, Trash2, Settings, ChevronDown, ChevronUp, FileText } from 'lucide-react';
import {
  useTimeOffPolicies,
...
export function PolicyBuilder() {
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      try {
        ...
        toast({
          title: 'Error',
          description: 'Failed to delete policy. It may have balances assigned.',
          variant: 'destructive',
        });
      }
    },
  ...
  const toggleCategory = useCallback((cat: TimeOffCategory) => {
    setFormData((f) => ({
      ...
      categories: f.categories?.includes(cat)
        ? f.categories.filter((c) => c !== cat).length === 0
          ? [cat]
  ...
  const addMilestoneTier = useCallback(() => {
    setFormData((f) => ({
      ...
      milestone_tiers: [
        ...(f.milestone_tiers || []),
        { after_months: 12, accrual_hours: 1, accrual_per_hours_worked: 20 },
      ],
    }));
  }, []);
            ...
            <CardContent className="space-y-4 pt-4">
              <div className="space-y-1.5">
                <Label style={{ color: colors.brown }}>
                  Policy Name <span style={{ color: colors.red }}>*</span>
                </Label>
                <Input
                  value={formData.name}
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (v === '' || /^\d*\.?\d*$/.test(v))
                          setFormData((f) => ({ ...f, accrual_hours: v === '' ? 0 : Number(v) }));
                      }}
                      className="w-20"
                      style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                    />
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        const v = e.target.value;
                        if (v === '' || /^\d+$/.test(v))
                          setFormData((f) => ({ ...f, accrual_per_hours_worked: v === '' ? 1 : Number(v) }));
                      }}
                      className="w-20"
                      style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                    />
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <Input
                      ...
                      onChange={(e) => {
                        const v = e.target.value;
                        if (v === '' || /^\d+$/.test(v))
                          setFormData((f) => ({ ...f, annual_hours: v === '' ? 0 : Number(v) }));
                      }}
                      className="w-24"
                      style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                    />
                    ...
                    >
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === '' || /^\d+$/.test(v))
                            updateMilestoneTier(idx, 'after_months', v === '' ? 0 : Number(v));
                        }}
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === '' || /^\d*\.?\d*$/.test(v))
                            updateMilestoneTier(idx, 'accrual_hours', v === '' ? 0 : Number(v));
                        }}
                        className="w-16 h-8 text-sm"
                        style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                      />
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === '' || /^\d+$/.test(v))
                            updateMilestoneTier(idx, 'accrual_per_hours_worked', v === '' ? 1 : Number(v));
                        }}
                        className="w-16 h-8 text-sm"
                        style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                      />
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => removeMilestoneTier(idx)}
              ...
              {formData.policy_type === 'none' && (
                <div className="p-3 rounded-lg" style={{ backgroundColor: colors.white }}>
                  <p className="text-sm" style={{ color: colors.brownLight }}>
                    No PTO will be accrued for this policy. Employees will not accumulate time-off hours. Use this if
                    your shop does not offer paid time off.
                  </p>
                </div>
              )}
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === '' || /^\d+$/.test(v))
                            setFormData((f) => ({ ...f, max_balance_hours: v === '' ? null : Number(v) }));
                        }}
                        className="w-32"
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const v = e.target.value;
                            if (v === '' || /^\d+$/.test(v))
                              setFormData((f) => ({ ...f, carryover_max_hours: v === '' ? 0 : Number(v) }));
                          }}
                          className="w-24"
                          style={{ backgroundColor: colors.inputBg, borderColor: colors.gold }}
                        />
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === '' || /^\d+$/.test(v))
                            setFormData((f) => ({ ...f, waiting_period_days: v === '' ? 0 : Number(v) }));
                        }}
                        className="w-24"
                        style={{ backgroundColor: colors.inputBg, borderColor: colors.creamDark }}
                      />
                      <span className="text-sm" style={{ color: colors.brownLight }}>
                        days after hire before accrual starts
                      </span>
                    </div>
                  </div>
                </>
